...
After some plotting of varibles wich we can see in fig.\@ref(fig:HPIall) we came to the conclution that equation $\eqref{HPI}$. would be a good Structual model for tha Housing Price Index (*HPI*). We will find out after some further analysis in later tasks if this prooves a strong model for the HPI. We belive that interest rates (*IR*) is a key indicator of (*HPI*). the (*I*) inflation should be a strong indicator aswell but might be harder to use for the fact the if inflation is high people tend to try to invest in realestat but in crontrast to that that might be hard to do since puchasing power is less during periods of high inflation. Two other varibles we belive play a big roll is finished housing (*HF*) and (*CC*) construction cost as it should reflect the classic case of supply and demand to the model. So $\beta_1$ should be negative as if interest rates are high we would se a drop in housing price(however this is hard to determine), $\beta_2$ is hard to determine if it is positive or negative as purchasing of housing can might seem like the right thing to do during high inflation but prooves hard, we belive that it in hovever negative. $\beta_3$ is most likly negative for the fact that if we flood the market with new apartments and demant gos down so will tha (*HPI*). Constructioncost $\beta_4$ is positive as it only makes sens tha if construction costs are high tha (*HPI*) well follow.
...
The top plot of fig.\@ref(fig:HPIall) we se the how tha housing price index and lease index develop over the period to be analysed. judjing by this graph only we would determin that there not an endogenious relationship between tha variables. They seem to follow similar paths until beguining of 2005 where they part ways. after the the crises we see a big dropp in housing prices but tha lease index seems to only drop slightly but keep rising steddy. tha lease index seem to not be as sensitive to the bubble.
...
```
If wee look back at fig.\@ref(fig:HPIall) and have a look at variables other than the lease index it most of them seem to show some correlation but would need further analysis to determin if they would be a signigicant causal variable to tha HPI.

## Task 3

To make a model of tha data we started by including all the variable in tha model. and after some tryal and error investigation removed some of them. bellow we can se tha fit of tha full model alogn with the residual from that model
...
After removing data and only including tha data we belive is usefull for the model we get the following summary. If we compare ther residual plot of fig.\@ref(fig:Housingfit) and fig.\@ref(fig:Housingfitpartial) we seem to increas tha residual error and from that we concluded that reducing the data in this case is not benefiticial oposite to what is happening in tha salesmodel in later in this project. We also would have guesst that the puchasing power and purchasing power of wages would be giving tha same information to the model and we would get some varible inflation regarding the wo varibles bu that does not seem to be tha case.
...
As mention above we dont seem to improve tha model by reducing tha number of variables which might be a little strange compared to tha salesmodel where its seem efective, but to try to improve our model we introduced two new binary bariable. the first one called *BubbleStarts* is zerountil the beginning of year 2005 whene Íbíðarlánasjóður started offering 90% leverage lones fore the first time, at this timepoint the varible becoms 1. The next wariable is called *BubbleBursts* and is zero until October 2008 whene tha financial crises struck tha worl with its well known meltdown.
