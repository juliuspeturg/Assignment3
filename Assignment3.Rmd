...
After some reserch on stock prices we belive the gross domestic priduct (*GDP*) would be one causal variable for the stock price index (*SPI*). Althog we currently have restriction og currency exhance here in iceland we belive that in an unrestricted market in a small country like iceland tha exchange rate (*ER*) would have a big impact on the SPI, specialy for companies in tha export buisness. Another causal variable would be the inflation (*I*) which effects the room for expansion of companies. The last but probably the most importan one would be the companies overal performance (*COP*) this varibal is estimatat according to standart procedures regarid the financial ratios of tha company in question for example *dept coverage ratio* etc. so the strucual equation should look like $\eqref{SPI}$.
...
We belive that $\beta_1$,$\beta_4$, are positive but $\beta_2$,$\beta_2$, are negative as inflation and exchange rate would have negative effects on the SPI.  


## Scandinavian electricity market

After reading throug some of the information on http://www.nordpoolspot.com/How-does-it-work/ we concluded tha there are the price seems to be tha classic case of supply (S) and demand (D). the weather (W) plays a big roll in the price but this however most likley is correlated with the demand so it might provide tha same information as the demand variable. 
...
We belive that $\beta_1$, is positive since if the demand increasas the obviusy will result in a higer price of electricity and visa versa for $\beta_2$. the weather variable should be negative, due to the fact that good weather shoude reduce the consumptuin of electriciti in house heating etc.  


## Inflation

It seems hard to find concreta data on http://www.hagstofan.com/ regardin how the inflation is evaluated bu it seems that inflation based on Consumer Price Index (CPI), wage index(WI) Cost Of Living (COL) and Money Supply (MS). as increas in all the indexes would result in higer inflation all the $\beta$ constans should be positive.
...
For this task we are to construct a salesmodel from the provided dataset. The datesets consists of around 20 variables wich can be used to construct a model for the sales. After some very basis investigation of what the datamodel consists of we come to the conclution of using 4 varibles in tha salesmodel. The first variable (*SE*) is the sesonal variable. tha seasonal varable should account for seasonality in tha data in respect to both the week of year and week of month. We belive tha customers should be more willing to spend money in the first week of the monthe due to a more positive ballance on tha bank account of being payed frome there emplyeer. In respect to the week of year seasonality we should se some classic patterns of consumers for exaple less willingness to purchase after the christmas season and therefor the sign of this variable should be positive. The second varible is the unemployment rate called (*E*) as it should reflect the current economic status of the consumers. It is known that in with higher rates of unemployment the tha purchasing power goes down so this varaible should give insight to the purchasing power and therefor the sign of the economic variable should be negetive(with respect to unemplyment rate). The third varible is weather varible (*W*), we belive that tha wether will have a effect on the sales, we do have some weather parameters in tha data and will have to decide on wich to use with some futher analysis so the sign of the weather variable is hard to estimate without further analysis. The last varable (*M*) is the marketing varible this one should be positive as there should be positive correlation between brandi the store and spending in tha store. Below we can see the salesmodel in equation $\eqref{eq1}$.
...
The purpos of this task is to get familiar whith tha dataset we are working with. The obvius first step is simply to plot the sales of the buildingsupply store which we can see in fig.\@ref(fig:chunk1). below we go into further analysis step by step.Hopfully we vwill ba able to end up with a model that follows tha structual eqaution of $\eqref{eq1}$. by combining and droping irrelevent data.
...
Next we plotted the sales per week of year along with the smoothed value to se the monthly variations in sales, as we see in fig.\@ref(fig:monthlySales) Sales seem to bee dropping in the later part of the year but that is due to the data lacking the later halft of the year 2009. 
...
Now its goot to go some baskic priliminary analysis like ACF and PACF plots of tha data, as we can see in fig.\@ref(fig:salesACF) there is a clear pattern in tha plots so hopfully we will be able to brake down the data and make a good model  
...
The next step was to make a full model usin all the available parameters in tha data, We will hopfully be able to improve ths model with the methods descussed in lecturs. The full model in fig.\@ref(fig:ACFresidual) will serve as tha benchmark to beat. tha aime is to reduce tha parameters needed to derive a model and therafor reduce the posabillity of varibal inflation and other negative aspects of using all the parameters in a sigle model.
...
```{r ACFresidual, eval=T, echo=F,message=FALSE, fig.cap=list("","ACF of residual for the first basic Linnear regression model (top) and tha fit of the model (bottom)"),fig.align='center'}
...
## Task 3
To begin this tas it is recomanded to plot all the Media spending fig.\@ref(fig:Mediaspending) of the retailer. This is done to get a vizualisation of what we are working with. As we are trying to make new variable that accounts for total media spenging and the assumed effect of each of the types. There will be að decaing effect of each of the catagories of media and from the inital plot we can try to guess tha decaing parameter to smooth out the new media variable called Media.adstock and hopfully get an significant new variable.
...
```{r residplot, eval=TRUE, echo=F,message=FALSE,fig.cap=list("","The residual plot and tha ACF af the residual"), fig.align='center'}
...
## Task 4
To make tha best model fit we decided to make one Weater varable with a weighted average of all the wather data, this is done in a similar manner as tha optimization in task 3. we ar going to drop the competitiorspendinras itdoes notseem to improve the model as vell as add two newbinari variable to indicate if it is summer or christmas season.
...
```{r monthplot, eval=TRUE, echo=F,message=FALSE,fig.cap=list("The final model. we can se it fits tha data a lot better then the first model"), fig.align='center'}
